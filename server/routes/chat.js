...
router.post("/chat", chatLimiter, async (req, res) => {
  ...
  try {
    ...
    // Client may have left while we waited for Gemini to start — bail before
    // streaming so we never drain or bill an abandoned turn.
    ...
    // Hard wall-clock cap on the read. A TIMER (not just a per-iteration clock
    // check) is required because a mid-stream stall blocks reader.read() with no
    // data arriving — cancelling the reader unblocks that pending read so the cap
    // actually fires instead of hanging to undici's multi-minute body timeout.
    ...
    // Abandoned mid-stream → don't bill or persist a turn the user walked away from.
    if (clientGone) return;
