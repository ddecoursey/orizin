...
export async function geminiGenerateJson({ system, prompt, schema, temperature = 0.45, models, leadModel }) {
  ...
  const ladder = models && models.length ? models : [valueModel(), liteModel()];

  // Ordered (key, model) attempts: leadModel once on the primary key, then the
  // ladder on the primary key, then the ladder on the backup key.
  const combos = [];
  if (leadModel) combos.push({ apiKey: keys[0], model: leadModel });
