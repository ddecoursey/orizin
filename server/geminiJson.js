...
// NODE_ENV=production, which must not silently enable paid models or background
// generations.
export function isProductionEnv() {
  ...
  const paypalEnv = String(process.env.PAYPAL_ENV || "").trim().toLowerCase();
  if (paypalEnv) return paypalEnv === "live";
...
// Autonomous Lite inference is opt-in even in production. User-initiated Lite
// requests remain available; this only controls the hourly screener sweep.
export const backgroundLiteIntangiblesEnabled = () =>
  isProductionEnv() && process.env.SCREENER_INTANGIBLES_ENABLED === "true";
...
async function runLadder(baseBody, combos, cacheOpts = {}, { deadline = 0, maxAttempts = ladderMaxAttempts() } = {}) {
  ...
  for (let attempt = 0; attempt < limit; attempt++) {
    ...
    if (res && res.ok) {
      ...
      if (!text) {
        throw err("Ori returned an empty response", "bad_json", { billableGeneration });
      }
      try {
        ...
        throw err("Ori returned malformed JSON", "bad_json", { billableGeneration });
      }
    }
