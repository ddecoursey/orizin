...
test("sanitizeLiteIntangibles returns screener-safe shape", () => {
  ...
  assert.equal(out.intangiblesScore, 35);
  assert.equal(out.xFactors.length, 0);
  assert.equal(out.convictionDelta, 8);
...
test("xFactors are derived from categoryScores — sorted by score, 'none' dropped", () => {
  const out = sanitizeLiteIntangibles({
    ...
    categoryScores: {
      ...
      pricing_power_distribution: { score: 60, rating: "moderate", note: "some lock-in" },
...
test("intangiblesScore is derived from categoryScores when the model omits it", () => {
  const out = sanitizeLiteIntangibles({
    ...
    categoryScores: {
      ...
      pricing_power_distribution: { score: 15, rating: "none" },
