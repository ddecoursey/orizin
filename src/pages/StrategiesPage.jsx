        ...
        <div className="mx-auto grid max-w-5xl gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.7fr)]">
          ...
          <aside className="min-w-0 space-y-4">
            <section className="min-w-0 rounded-2xl border border-emerald-900/40 bg-emerald-950/10 p-4"><div className="flex items-center gap-2"><Icon name="shield" className="h-4 w-4 text-emerald-400" /><h3 className="text-sm font-semibold text-gray-100">Safety limits</h3></div><p className="mt-1 text-[10px] leading-4 text-gray-500">These are always fixed rules, even when Ori helps.</p><div className="mt-4 space-y-3">
              <label className="block"><span className="text-[9px] font-semibold text-gray-500">Check the market</span><select value={draft.limits.rebalance} onChange={(event) => updateDraft((current) => ({ ...current, limits: { ...current.limits, rebalance: event.target.value } }))} className="mt-1.5 w-full rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-xs text-gray-300 outline-none focus:border-cyan-700">{["Daily", "Weekly", "Every 2 weeks", "Monthly", "Quarterly"].map((value) => <option key={value}>{value}</option>)}</select></label>
              <div className="grid grid-cols-2 gap-2"><label className="min-w-0"><span className="text-[9px] font-semibold text-gray-500">Most holdings</span><input type="number" min="1" max="20" value={draft.limits.maxPositions} onChange={(event) => updateDraft((current) => ({ ...current, limits: { ...current.limits, maxPositions: event.target.value } }))} className="mt-1.5 w-full min-w-0 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-xs text-gray-300 outline-none focus:border-cyan-700" /></label><label className="min-w-0"><span className="text-[9px] font-semibold text-gray-500">Max per holding</span><div className="relative mt-1.5 min-w-0"><input type="number" min="3" max="100" value={draft.limits.maxPositionPct} onChange={(event) => updateDraft((current) => ({ ...current, limits: { ...current.limits, maxPositionPct: event.target.value } }))} className="w-full min-w-0 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 pr-7 text-xs text-gray-300 outline-none focus:border-cyan-700" /><span className="pointer-events-none absolute right-3 top-2 text-xs text-gray-600">%</span></div></label></div>
              <label className="block"><span className="text-[9px] font-semibold text-gray-500">Always keep in cash</span><div className="relative mt-1.5"><input type="number" min="0" max="90" value={draft.limits.cashReservePct} onChange={(event) => updateDraft((current) => ({ ...current, limits: { ...current.limits, cashReservePct: event.target.value } }))} className="w-full rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 pr-7 text-xs text-gray-300 outline-none focus:border-cyan-700" /><span className="absolute right-3 top-2 text-xs text-gray-600">%</span></div></label>
              <label className="block"><span className="text-[9px] font-semibold text-gray-500">Starting paper money</span><input type="number" min="1000" step="1000" value={draft.paper.startingCash} onChange={(event) => updateDraft((current) => ({ ...current, paper: { ...current.paper, startingCash: event.target.value, cash: current.paper.holdings.length ? current.paper.cash : event.target.value } }))} disabled={!isNew || draft.paper.holdings.length > 0} className="mt-1.5 w-full rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-xs text-gray-300 outline-none focus:border-cyan-700 disabled:opacity-50" /></label>
