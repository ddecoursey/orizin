...
export default function PortfolioGoalsPage({ stocks = [], theme = 'dark', onSelectStock, detailStock }) {
  ...
  const handleTickerClick = (ticker) => { if (!onSelectStock) return; const stock = stocks.find(s => s.symbol === ticker.toUpperCase()); if (stock) onSelectStock(stock); };
  const formatMoney = (n) => (n || 0).toLocaleString(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 });
            ...
            {overallAllocations.length > 0 && (
              <div className="rounded-2xl border border-gray-800 bg-gray-900/50 p-5 mb-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {overallAllocations.slice(0, 10).map((alloc) => (
                    <div key={alloc.ticker} onClick={() => handleTickerClick(alloc.ticker)} className={`bg-gray-950 border rounded-xl p-3 flex flex-col gap-1 cursor-pointer transition-all hover:bg-gray-900 ${detailStock?.symbol === alloc.ticker ? 'border-blue-500 ring-1 ring-blue-500/50' : 'border-gray-800'}`}>
                      <div className="flex items-center justify-between"><span className="font-mono font-bold text-gray-100">{alloc.ticker}</span><span className="text-emerald-400 font-mono text-xs">{alloc.overallPercent.toFixed(1)}%</span></div>
                  ...
                  <div className="border border-gray-800 rounded-xl overflow-hidden bg-gray-950">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-800">
                        {(selectedPortfolio.holdings || []).length === 0 && <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-500 text-sm">No holdings yet.</td></tr>}
