      ...
      <aside className="relative w-full max-w-sm bg-gray-950 border-l border-gray-800 flex flex-col shadow-2xl oz-pane-in">
        <header className="flex items-center gap-2 px-4 py-3 border-b border-gray-800 shrink-0">
          ...
          <select
            ...
            className="ml-auto text-xs bg-gray-900 border border-gray-700 rounded-md px-2 py-1 text-gray-200 outline-none"
          >
            {lists.map((w) => (
              <option key={w.id} value={w.id}>{w.name} ({w.symbols.length})</option>
            ))}
          </select>
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-gray-200 px-1 cursor-pointer">×</button>
        </header>
        ...
        <div className="px-4 py-2 border-b border-gray-800 flex gap-2 shrink-0">
          {adding ? (
            <input
              ...
              maxLength={28}
              className="flex-1 text-xs bg-gray-900 border border-gray-700 rounded-md px-2 py-1.5 text-gray-100 outline-none focus:border-violet-500/60"
            />
          ) : (
            <button
              type="button"
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {symbols.length === 0 ? (
            <p className="text-xs text-gray-500 text-center py-10 px-4">
              Star stocks in the screener or add from Deep Research to build your {activeList?.name || "watchlist"}.
            </p>
          ) : (
        ...
        <footer className="px-4 py-2 border-t border-gray-800 text-[10px] text-gray-600 shrink-0">
          {canUseOri ? "Sorted by conviction in screener" : "Orizin Score shown · upgrade for conviction"}
        </footer>
      </aside>
    </div>
  );
